...
App.prototype = {
	...
	_generateCommand: function() { 	
		...
		var command  = 'export PATH=$PATH:/usr/local/bin';
			...
			command += this.project.mongoUrl ? ' MONGO_URL='+this.project.mongoUrl : ''; 
	...
	syncEvents: function(iframe) {
		...
		this.enlargeIframeOnHover(iframe);
	},
	enlargeIframeOnHover: function(iframe) {
		$(window).resize(function() {
			$(iframe).css('height', $('.nucleus-frame').height() - 40);
		});
		
		$('.nucleus-frame').hover(function() {
			$('.nucleus-frame, .nucleus-frame iframe').css('width', 700);
			$('.close-button, .refresh-button').css('width', 350);
		}, function() {
			$('.nucleus-frame, .nucleus-frame iframe').css('width', 400);
			$('.close-button, .refresh-button').css('width', 200);
		});
	},
